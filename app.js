...
(function () {
  ...
  /* ---------------- audience mode (Student / Teacher / Researcher) ---------------- */
  let mode = localStorage.getItem("atlas-mode") || "student";
  const MODE_ORDER = ["student", "teacher", "researcher"];
  // each mode re-ranks which resource categories surface first (brief §14)
  const MODE_CAT_RANK = {
    student:    ["Theory & Notes", "Video Lectures", "Problems & Solutions", "Papers, PDFs & Slides", "Tools", "Community"],
    teacher:    ["Papers, PDFs & Slides", "Theory & Notes", "Problems & Solutions", "Tools", "Video Lectures", "Community"],
    researcher: ["Theory & Notes", "Papers, PDFs & Slides", "Tools", "Problems & Solutions", "Community", "Video Lectures"],
  };
  const catRankIndex = (key) => { const r = MODE_CAT_RANK[mode] || MODE_CAT_RANK.student; const i = r.indexOf(key); return i === -1 ? 99 : i; };
  // within a category: editor picks first, then direct pages, then searches
  const resWeight = (r) => (r._pick ? 0 : 2) + (r._search ? 1 : 0);

  // honest, transparent coverage score (brief §16) — surfaces thin chapters
  function coverage(ch) {
    const res = (ch._res || []).filter((r) => !r.feature);
    const total = res.length;
    const direct = res.filter((r) => !r._search).length;
    const tools = res.filter((r) => r.cat === "Tools").length;
    const sources = new Set(res.map((r) => r.source)).size;
    const cats = new Set(res.map((r) => r.cat)).size;
    const subs = (ch.subtopics || []).length;
    const clamp = (x) => Math.max(0, Math.min(1, x));
    const score = Math.round(100 * (
      0.25 * (cats / 6) +            // section breadth
      0.20 * clamp(sources / 12) +   // source diversity
      0.20 * clamp(total / 60) +     // volume
      0.15 * clamp(subs / 6) +       // subtopic richness
      0.20 * clamp(direct / 12)      // verified-direct depth (low = needs real resources)
    ));
    return { score, total, direct, search: total - direct, tools, sources, cats, subs };
  }
  ...
  const I18N = {
    en: { nav_chapters: "Chapters", nav_howto: "How to use", nav_about: "About", nav_feedback: "Feedback", hero_eyebrow: "● Free · trusted sources only · for learners everywhere", hero_h1a: "The world's best problems & resources,", hero_h1b: "mapped chapter by chapter.", hero_lede: "One tap to the best free resources on any chapter — Maths, Physics, Chemistry & Biology, from school basics to Olympiad. Trusted sources only, links you can rely on.", hero_search: "Search chapters — e.g. limits, rotation, genetics, electrochemistry…", stat_chapters: "Chapters", stat_links: "Curated links", stat_trusted: "Trusted sources", stat_free: "Forever free", guide_kicker: "Start here", guide_h2: "How to use this — in 30 seconds", guide_sub: "No account, no install. Built for teachers who just want great resources, fast.", s1t: "Pick subject & chapter", s1d: "Choose Maths / Physics / Chemistry / Biology, then tap a chapter — or just search.", s2t: "Tap a section", s2d: "PDFs, Problems, Videos, Theory, Community or Tools — it jumps straight there, no scrolling.", s3t: "Filter by level", s3d: "Narrow to Foundation, Standard, Advanced or Olympiad. Open or copy any link.", s4t: "Need more?", s4d: "Every chapter links to an AI prompt library — generate unlimited fresh problems in one click.", callout: "Only sources you can trust — universities, the Stack Exchanges, AoPS, government textbooks and renowned creators. No random sites — every link is a live search or an official page from a source you can trust.", atlas_kicker: "The Atlas", atlas_h2: "Every chapter, every level", atlas_sub: "{0} {1} chapters · grouped the way you teach them.", search_match: "{0} chapter(s) match “{1}” across all subjects.", res_word: "resources", weight_word: "weight", crumb_all: "← All chapters", m_subject: "Subject", m_group: "Group", m_weight: "Weight", m_resources: "Resources", filter_ph: "Filter within this chapter…", btn_copy: "⧉ Copy visible links", btn_share: "▦ Share / QR", btn_reset: "Reset", level_word: "Level", start_here: "Start here", sh_learn: "Learn it", sh_watch: "Watch", sh_practise: "Practise", sh_papers: "Past papers", sh_explore: "Explore", open_label: "Open ↗", search_label: "Search ↗", lt_search: "⌕ Search", lt_direct: "Direct", mode_label: "I am a", mode_student: "Student", mode_teacher: "Teacher", mode_researcher: "Researcher", mode_hint_student: "Ranked for learning: theory → videos → practice.", mode_hint_teacher: "Ranked for teaching: slides, past papers, multiple methods.", mode_hint_researcher: "Ranked for depth: lecture notes, references, tools.", cov_label: "Coverage", cov_break: "{0} direct · {1} searches · {2} tools · {3} sources · {4}/6 sections", showing: "{0} of {1} resources · showing {2}", share_sub: "Project the QR for students to scan, or copy the link.", share_copy: "⧉ Copy link", share_more: "↗ Share…", empty_filter: "No matches — try clearing the level filter." },
    hi: { nav_chapters: "अध्याय", nav_howto: "कैसे उपयोग करें", nav_about: "परिचय", nav_feedback: "प्रतिक्रिया", hero_eyebrow: "● निःशुल्क · केवल विश्वसनीय स्रोत · सभी के लिए", hero_h1a: "दुनिया के बेहतरीन प्रश्न और संसाधन,", hero_h1b: "अध्याय दर अध्याय।", hero_lede: "किसी भी अध्याय के सर्वोत्तम निःशुल्क संसाधन एक टैप में — गणित, भौतिकी, रसायन और जीव विज्ञान, स्कूल से ओलंपियाड तक। केवल विश्वसनीय स्रोत, भरोसेमंद लिंक।", hero_search: "अध्याय खोजें — जैसे limits, rotation, genetics…", stat_chapters: "अध्याय", stat_links: "चयनित लिंक", stat_trusted: "विश्वसनीय स्रोत", stat_free: "हमेशा निःशुल्क", guide_kicker: "यहाँ से शुरू करें", guide_h2: "उपयोग कैसे करें — 30 सेकंड में", guide_sub: "न खाता, न इंस्टॉल। शिक्षकों के लिए, जिन्हें तेज़ी से बेहतरीन संसाधन चाहिए।", s1t: "विषय और अध्याय चुनें", s1d: "गणित / भौतिकी / रसायन / जीव विज्ञान चुनें, फिर अध्याय टैप करें — या खोजें।", s2t: "सेक्शन टैप करें", s2d: "PDF, प्रश्न, वीडियो, सिद्धांत, समुदाय या उपकरण — सीधे वहीं, बिना स्क्रॉल।", s3t: "स्तर से छाँटें", s3d: "आधारभूत, मानक, उन्नत या ओलंपियाड चुनें। कोई भी लिंक खोलें या कॉपी करें।", s4t: "और चाहिए?", s4d: "हर अध्याय में AI प्रॉम्प्ट लाइब्रेरी — एक क्लिक में असीमित नए प्रश्न बनाएँ।", callout: "केवल भरोसेमंद स्रोत — विश्वविद्यालय, Stack Exchange, AoPS, सरकारी पाठ्यपुस्तकें और प्रसिद्ध रचनाकार। कोई रैंडम साइट नहीं — हर लिंक एक लाइव खोज या किसी भरोसेमंद स्रोत का आधिकारिक पेज।", atlas_kicker: "एटलस", atlas_h2: "हर अध्याय, हर स्तर", atlas_sub: "{0} {1} अध्याय · जैसे आप पढ़ाते हैं वैसे समूहित।", search_match: "{0} अध्याय “{1}” से मेल खाते हैं (सभी विषयों में)।", res_word: "संसाधन", weight_word: "महत्त्व", crumb_all: "← सभी अध्याय", m_subject: "विषय", m_group: "समूह", m_weight: "महत्त्व", m_resources: "संसाधन", filter_ph: "इस अध्याय में खोजें…", btn_copy: "⧉ दिखाए लिंक कॉपी करें", btn_share: "▦ शेयर / QR", btn_reset: "रीसेट", level_word: "स्तर", start_here: "यहाँ से शुरू करें", sh_learn: "सीखें", sh_watch: "देखें", sh_practise: "अभ्यास", sh_papers: "पुराने पेपर", sh_explore: "एक्सप्लोर", open_label: "खोलें ↗", search_label: "खोजें ↗", lt_search: "⌕ खोज", lt_direct: "सीधा", mode_label: "मैं हूँ", mode_student: "विद्यार्थी", mode_teacher: "शिक्षक", mode_researcher: "शोधकर्ता", mode_hint_student: "सीखने के क्रम में: सिद्धांत → वीडियो → अभ्यास।", mode_hint_teacher: "पढ़ाने के क्रम में: स्लाइड, पुराने पेपर, कई विधियाँ।", mode_hint_researcher: "गहराई के क्रम में: लेक्चर नोट्स, संदर्भ, उपकरण।", cov_label: "कवरेज", cov_break: "{0} सीधे · {1} खोज · {2} उपकरण · {3} स्रोत · {4}/6 खंड", showing: "{1} में से {0} संसाधन · {2} दिखा रहे हैं", share_sub: "QR प्रोजेक्ट करें ताकि विद्यार्थी स्कैन करें, या लिंक कॉपी करें।", share_copy: "⧉ लिंक कॉपी करें", share_more: "↗ शेयर…", empty_filter: "कोई मेल नहीं — स्तर फ़िल्टर हटाकर देखें।" },
    es: { nav_chapters: "Capítulos", nav_howto: "Cómo usar", nav_about: "Acerca de", nav_feedback: "Comentarios", hero_eyebrow: "● Gratis · solo fuentes confiables · para todos", hero_h1a: "Los mejores problemas y recursos del mundo,", hero_h1b: "capítulo por capítulo.", hero_lede: "Un toque para los mejores recursos gratuitos de cualquier capítulo — Matemáticas, Física, Química y Biología, desde lo básico hasta la Olimpiada. Solo fuentes confiables en las que puedes confiar.", hero_search: "Busca capítulos — p. ej. limits, rotation, genetics…", stat_chapters: "Capítulos", stat_links: "Enlaces", stat_trusted: "Fuentes confiables", stat_free: "Siempre gratis", guide_kicker: "Empieza aquí", guide_h2: "Cómo usarlo — en 30 segundos", guide_sub: "Sin cuenta, sin instalar. Hecho para docentes que quieren buenos recursos, rápido.", s1t: "Elige materia y capítulo", s1d: "Elige Matemáticas / Física / Química / Biología y toca un capítulo — o busca.", s2t: "Toca una sección", s2d: "PDF, Problemas, Videos, Teoría, Comunidad o Herramientas — va directo, sin desplazar.", s3t: "Filtra por nivel", s3d: "Básico, Estándar, Avanzado u Olimpiada. Abre o copia cualquier enlace.", s4t: "¿Más?", s4d: "Cada capítulo enlaza una biblioteca de prompts de IA — genera problemas nuevos en un clic.", callout: "Solo fuentes confiables — universidades, los Stack Exchange, AoPS, libros oficiales y creadores reconocidos. Sin sitios al azar — cada enlace es una búsqueda activa o una página oficial de una fuente confiable.", atlas_kicker: "El Atlas", atlas_h2: "Cada capítulo, cada nivel", atlas_sub: "{0} capítulos de {1} · agrupados como los enseñas.", search_match: "{0} capítulo(s) coinciden con «{1}» en todas las materias.", res_word: "recursos", weight_word: "peso", crumb_all: "← Todos los capítulos", m_subject: "Materia", m_group: "Grupo", m_weight: "Peso", m_resources: "Recursos", filter_ph: "Filtrar en este capítulo…", btn_copy: "⧉ Copiar enlaces visibles", btn_share: "▦ Compartir / QR", btn_reset: "Restablecer", level_word: "Nivel", start_here: "Empieza aquí", sh_learn: "Aprende", sh_watch: "Mira", sh_practise: "Practica", sh_papers: "Exámenes", sh_explore: "Explora", open_label: "Abrir ↗", search_label: "Buscar ↗", lt_search: "⌕ Búsqueda", lt_direct: "Directo", mode_label: "Soy", mode_student: "Estudiante", mode_teacher: "Docente", mode_researcher: "Investigador", mode_hint_student: "Ordenado para aprender: teoría → videos → práctica.", mode_hint_teacher: "Ordenado para enseñar: diapositivas, exámenes, métodos.", mode_hint_researcher: "Ordenado por profundidad: apuntes, referencias, herramientas.", cov_label: "Cobertura", cov_break: "{0} directos · {1} búsquedas · {2} herramientas · {3} fuentes · {4}/6 secciones", showing: "{0} de {1} recursos · mostrando {2}", share_sub: "Proyecta el QR para que los alumnos lo escaneen, o copia el enlace.", share_copy: "⧉ Copiar enlace", share_more: "↗ Compartir…", empty_filter: "Sin resultados — quita el filtro de nivel." },
  };
  ...
  function renderChapter(slug) {
    ...
    app.innerHTML = `
      <section class="detail wrap" style="--subj:${subjAccent}">
        ...
        <div class="coverage" data-score="${cov.score}">
          <div class="cov-top"><span class="cov-label">${esc(t("cov_label"))}</span><span class="cov-pct">${cov.score}%</span></div>
          <div class="cov-bar"><span style="width:${cov.score}%"></span></div>
          <div class="cov-break">${esc(t("cov_break", cov.direct, cov.search, cov.tools, cov.sources, cov.cats))}</div>
        </div>

        ${(() => { const picks = startHerePicks(res); return picks.length ? `<div class="starthere"><div class="sh-head"><span aria-hidden="true">✦</span> ${esc(t("start_here"))}</div><div class="sh-row">${picks.map((p) => `<a class="sh-card" href="${esc(p.url)}" target="_blank" rel="noopener noreferrer"><span class="sh-cat">${esc(t(SH_LABEL[p.cat] || "sh_explore"))}</span><span class="sh-title">${esc(p.title.split(" — ").pop())}</span><span class="sh-why">${esc((p.note || "").slice(0, 78))}${(p.note || "").length > 78 ? "…" : ""}</span><span class="sh-go">${p._search ? esc(t("search_label")) : esc(t("open_label"))}</span></a>`).join("")}</div></div>` : ""; })()}
        ...
        <div class="toolbar">
          <div class="mode-switch" role="group" aria-label="${esc(t("mode_label"))}">
            <span class="ms-label">${esc(t("mode_label"))}</span>
            <div class="ms-seg">${MODE_ORDER.map((m) => `<button class="ms-btn${m === mode ? " active" : ""}" data-mode="${m}" type="button">${esc(t("mode_" + m))}</button>`).join("")}</div>
            <span class="ms-hint">${esc(t("mode_hint_" + mode))}</span>
          </div>
    ...
    app.querySelectorAll(".ms-btn").forEach((b) => b.addEventListener("click", () => {
      mode = b.dataset.mode; localStorage.setItem("atlas-mode", mode);
      app.querySelectorAll(".ms-btn").forEach((x) => x.classList.toggle("active", x.dataset.mode === mode));
      const hint = app.querySelector(".ms-hint"); if (hint) hint.textContent = t("mode_hint_" + mode);
      applyDetailFilters();
    }));
  ...
  function applyDetailFilters() {
    ...
    // mode-aware ranking: best resources first within a category, best categories first
    Object.keys(byCat).forEach((k) => byCat[k].sort((a, b) => resWeight(a) - resWeight(b)));
    const presentCats = CATS.filter((c) => byCat[c.key]).sort((a, b) => catRankIndex(a.key) - catRankIndex(b.key));
